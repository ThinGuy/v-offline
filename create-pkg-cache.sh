...
for DIST in ${VERSA_DISTS[@]};do
	eval cat ${VERSA_POOL_LISTS}/pool.files.${DIST}.txt|xargs -rn1 -P0 bash -c 'rsync -r --info=progress2 --info=name0 '${VERSA_ARCHIVES}'/us.archive.ubuntu.com/ubuntu/${0} '${VERSA_PKG_CACHE}'/${0}'
done
